...
4. **Stale display-name refresh** (after migration `0010_location_name_synced_at`): each successful GHL hydrate records `locations.location_name_synced_at`. The Worker cron re-queries GHL for locations whose last sync is older than **`LOCATION_NAMES_REFRESH_STALE_AFTER_DAYS`** (defaults to **1** day), including blank names (unnamed first, then oldest sync). One Worker invocation cannot fetch every subaccount, so the pass is rolling: **`*/15 * * * *`** does up to **8** lookups by default (`LOCATION_NAMES_REFRESH_BATCH`, `"0"` disables) and **`0 8 * * *`** adds a daily burst of **15** (`LOCATION_NAMES_DAILY_CRON_BATCH`, `"0"` disables, ignores **`LOCATION_NAMES_REFRESH_HOUR_UTC`**). Cap per tick is **15**. Do **not** put these batch vars in `wrangler.toml` `[vars]` (deploys overwrite Dashboard). **`POST /admin/locations/refresh-stale-names?limit=N`** triggers the same path manually.
...
5. **Automatic sync:** GHL webhooks **`SaaSPlanCreate`**, **`LocationCreate`**, **`LocationUpdate`**, and SaaS location/plan events (`saaslocation*`, `planchange`, etc.) queue a job that upserts the location, ensures a **`location_billing_config`** row, and syncs **`cus_…`** from the webhook payload or GHL SaaS API when **`STRIPE_SECRET_KEY`** is set. Skips when billing is already **ready** (`cus_` + payment method). The Worker cron (every 15 minutes) also backfills a small batch of subaccounts that still have no Stripe customer. Manual **Sync from GHL** and catalog backfill endpoints remain available.
...
7. **Live charges are opt-in:** Worker var **`CLIENT_CHARGES_CHARGING_ENABLED`** must be **`true`** before **`POST /workspace/client-charges/:appointmentId/charge`** or **retry** will run Stripe PaymentIntents. Set it in the **Cloudflare Worker dashboard**, not in `wrangler.toml` `[vars]` (those overwrite dashboard values on every deploy). Sync, customer link, and Checkout setup still work when it is unset/`false`.
