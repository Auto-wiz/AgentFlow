...
4. **GHL SaaS customer sync:** requires **SaaS Configurator** access on the agency. OAuth scopes **`saas/location.read`** / **`saas/company.read`** (and **`charges.readonly`** / **`charges.write`** on the current Marketplace version `6a6b5864ba65c71d784c0347`). Reconnect from Settings after publishing a new app version. Primary API: **`GET /saas/saas-locations/:companyId?page=1`** with header **`Version: v3`**, then match the subaccount `locationId` and read Stripe **`customerId`**. Fallback: legacy **`GET /saas/get-saas-subscription/:locationId?companyId=…`**. Admin endpoints:
   ...
   If sync returns **`customer_id_missing`**, the response includes **`payloadShape`** (keys/types only, no secrets) so the parser can be extended. **Connect GoHighLevel as Agency (`user_type=Company`)** after each Marketplace version publish — a single-subaccount reinstall stores a Location JWT that GHL rejects on SaaS APIs with **`403 Forbidden resource`** even when the DB snapshot lists `saas/*`. Enable SaaS for that location in HighLevel, then **Sync from GHL** again.
...
7. **Live charges:** Worker var **`CLIENT_CHARGES_CHARGING_ENABLED`** is pinned **`true`** in `apps/api/wrangler.toml` `[vars]` so deploys recreate it (a Dashboard-only plaintext var can disappear). Set it to **`false`** in `wrangler.toml` to pause **`POST /workspace/client-charges/:appointmentId/charge`**. Sync, customer link, and Checkout setup still work when it is unset/`false`.
