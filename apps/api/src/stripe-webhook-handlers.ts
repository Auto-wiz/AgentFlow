...
import { applyStripeAccountSnapshot, setLocationPaymentMethod } from "./location-billing-stripe.js";
...
async function handleCheckoutSessionCompleted(
  ...
  await handleSetupIntentSucceeded(env, db, setup);
}
  ...
  try {
    switch (event.type) {
      case "account.updated": {
        ...
        const locationId = account.metadata?.agentflow_location_id?.trim();
        if (locationId) {
      ...
      case "setup_intent.succeeded": {
        await handleSetupIntentSucceeded(c.env, db, event.data.object as Stripe.SetupIntent);
        break;
      }
